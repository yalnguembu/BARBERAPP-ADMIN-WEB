...
const ReservationDetails = () => {
  ...
  const cancelReservaion = async () => {
    if (Date.now() < new Date(reservation?.date as string).getTime())
      try {
        await ReservationService.cancelReservation({ id: id as string });
        navigate("/services/reservations");
      } catch (error) {
        console.log(error);
      }
    else alert("Cette reservation ne peux atre annulee");
